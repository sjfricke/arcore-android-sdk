  ...
  public:
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            else { LOGE( "ERROR: Got unexpected WebSocket message.\n"); close(); }
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
    ...
    char path[128];
    if (url.size() >= 128) {
      LOGE("ERROR: url size limit exceeded: %s\n", url.c_str());
      return NULL;
    }
    if (origin.size() >= 200) {
      LOGE("ERROR: origin size limit exceeded: %s\n", origin.c_str());
      return NULL;
    }
    if (false) { }
        ...
        LOGE( "ERROR: Could not parse WebSocket url: %s\n", url.c_str());
        return NULL;
    }
    LOGE( "easywsclient: connecting: host=%s port=%d path=/%s\n", host, port, path);
    socket_t sockfd = hostname_connect(host, port);
    if (sockfd == INVALID_SOCKET) {
        LOGE( "Unable to connect to %s:%d\n", host, port);
        return NULL;
    }
    {
        ...
        line[i] = 0;
        if (i == 255) { LOGE( "ERROR: Got invalid status line connecting to: %s\n", url.c_str()); return NULL; }
        if (sscanf(line, "HTTP/1.1 %d", &status) != 1 || status != 101) { LOGE( "ERROR: Got bad status connecting to %s: %s", url.c_str(), line); return NULL; }
